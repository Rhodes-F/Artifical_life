...
class NEURON: 
    ...
    def Update_Hidden_Or_Motor_Neuron(self):
            self.Set_Value(0.0)
            # for sourceNeuronName in self.sourceNeurons:
            #     sourceNeuron = self.sourceNeurons[sourceNeuronName]
            #     synapse = self.synapses[sourceNeuronName,self.name]
            #     self.Add_To_Value( synapse.Get_Weight() * sourceNeuron.Get_Value() )
            # self.Threshold()
